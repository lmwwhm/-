function cook(x) {
    ...
    var phone = x.cells[4].innerText;
    var 类型 = x.cells[5].innerText;
    var 备注 = x.cells[6].innerText;
    SetCookie("id", id, 1);
...
function people() {
    ...
    xmlHttp.onreadystatechange = function () {
        if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                var td = document.createElement('td'); //新建td
                td.innerHTML = "<td><a title='编辑' onclick='cook(this.parentNode.parentNode);xadmin.open(\"修改关系信息\", \"./people_update.html\", 800, 380)'><i class='layui-icon'>&#xe642;</i></a><a title='删除' onclick='people_del(this)' style='text-decoration:none'><i class='layui-icon'>&#xe640;</i></a></td>";
                tr.appendChild(td);
            }
        }
    ...
    var url = "/people/people?name=" + str1;
