function cook(x) {
    ...
    var timedur = x.cells[2].innerText;
    var todo = x.cells[3].innerText;

    SetCookie("id", id, 1);
    SetCookie("timedur", timedur, 1);
    ...
    xmlHttp.onreadystatechange = function () {
        if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                var td = document.createElement('td'); //新建td
                td.innerHTML = "<td><a title='编辑' onclick='cook(this.parentNode.parentNode);xadmin.open(\"修改时间段信息\", \"./timetable_update.html\", 800, 340)'><i class='layui-icon'>&#xe642;</i></a><a title='删除' onclick='timetable_del(this)' style='text-decoration:none'><i class='layui-icon'>&#xe640;</i></a></td>";
                tr.appendChild(td);
            }
        }
