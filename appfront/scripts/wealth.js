function cook(x) {
    ...
    var inwealth = x.cells[2].innerText;
    var outwealth = x.cells[3].innerText;
    var time = x.cells[4].innerText;
    var dowhat = x.cells[5].innerText;
    var 类型 = x.cells[6].innerText;
    SetCookie("id", id, 1);
    SetCookie("inwealth", inwealth, 1);
    SetCookie("outwealth", outwealth, 1);
...
function wealth() {
    ...
    xmlHttp.onreadystatechange = function () {
        if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                var td = document.createElement('td'); //新建td
                td.innerHTML = "<td></a><a title='编辑' onclick='cook(this.parentNode.parentNode);xadmin.open(\"修改财务信息\", \"./wealth_update.html\", 800, 280)'><i class='layui-icon'>&#xe642;</i></a><a title='删除' onclick='wealth_del(this)' style='text-decoration:none'><i class='layui-icon'>&#xe640;</i></a></td>";
                tr.appendChild(td);
            }
        }
    ...
    var url = "/wealth/wealth?start=" + str1 + "&end=" + str2;
